...
export default function App() {
  ...
  function processData(hourly, daily){
    ...
    //  filterAndFlagClosetTime() -> to get the current dates, hourly basis weather updates
  ...
  useEffect(() => {
    ...
    if("geolocation" in navigator){
      navigator.geolocation.getCurrentPosition((position) =>{
                                                              ...
                                                              fetch(`https://nominatim.openstreetmap.org/reverse.php?lat=${latitude}&lon=${longitude}&zoom=18&format=jsonv2`).then((response) => response.json()).then((location) => {
                                                                setForeCastLocation({
                                                                  label:`${location?.address?.village 
